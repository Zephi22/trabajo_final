  ...
  return (
    ...
    <QuizWindow>
            { trivia[numero] &&
                ...
                <h3 classNAme="bg-white text-center rounded-pill">{contador}</h3><br/>
                <ProgressBar now={contador*3.3333}/><br/>
                    <Question dangerouslySetInnerHTML={{ __html: trivia[numero].pregunta }}></Question>

                    <Options>
                        {trivia[numero].opciones.map((item, index) => (
                            <Option key={index} dangerouslySetInnerHTML={{ __html: item }} onClick={responderPregunta}></Option>
                        ))}
                    </Options>
                </>
